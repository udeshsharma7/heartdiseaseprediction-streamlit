...
def accur(df,x,y,x_train,y_train,x_test,y_test,x_std,x_train_std,x_test_std):
...
def prddd():
    ...
    opt= st.selectbox('Gender ', ('Male','Female'))
    if opt == 'Male':
        sexx=1
    elif opt == 'Female':
        sexx=0
...
if choice == "Home":
    ...
    st.subheader("Please Enter your Login ID and Password")
    ...
    if st.sidebar.checkbox('Login'):
        ...
        if result:
            ...
            task = st.selectbox('Select Task',['None','About Dataset','Want to see some Vistualization','Data Prepocessing','Check Acurracy of Algorithm','Want to Perform Pridiction'])
            if task == "About Dataset":
                dataset(df)

            elif task == "Want to see some Vistualization":
                vist(df)

            elif task == "Check Acurracy of Algorithm":
                accur(df,x,y,x_train,y_train,x_test,y_test,x_std,x_train_std,x_test_std)

            elif task == "Want to Perform Pridiction":
                st.warning("The inputs below are medical terms.So,you need to get those values for yourself and then insert them here")
                prddd()

            elif task == 'Data Prepocessing':
                datapre(df,x,y)
    ...
    st.subheader("Please Enter Details ")
